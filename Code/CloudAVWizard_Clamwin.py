...
def update():
    cmd = subprocess.Popen((config.get('modules_clamwin', 'pathtoupdater')))
    cmd.wait()
    return True

def scanfile(filename):
    cmd = subprocess.Popen((config.get('modules_clamwin', 'pathtoscanner'), '-i', os.path.join(config.get('modules_global', 'workingdir'), filename)), stdout=subprocess.PIPE)
    cmd.wait()
    result = re.findall(': (.*?) FOUND', cmd.stdout.read())
    if len(result) > 0:
        return result[0]
    else:
        return ''
...
locks = []
for a in xrange(0, config.getint('modules_clamwin', 'scanningthreads')):
...
while True:
    #If Time To Update
    if lastupdate < time.time() - config.getfloat('modules_clamwin', 'updateinterval'):
